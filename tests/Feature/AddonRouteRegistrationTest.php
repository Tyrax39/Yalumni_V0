...
{
    public function test_central_domain_helper_tracks_the_current_request_host(): void
    {
        config()->set('tenancy.central_domains', ['central.test']);

        $this->setRequestHost('central.test');
        $this->assertTrue(isCentralDomain());
        $this->assertSame('central.test', centralDomain());

        $this->setRequestHost('tenant.test');
        $this->assertFalse(isCentralDomain());
    }
    ...
    public function test_optional_addon_routes_are_not_registered_without_bootstrap_files(): void
    {
        ...
        $this->assertTrue(Route::has('super_admin.dashboard'));
        $this->assertNull(Route::getRoutes()->getByName('payment.verify')->domain());
        $this->assertNull(Route::getRoutes()->getByName('admin.dashboard')->domain());
        $this->assertNull(Route::getRoutes()->getByName('super_admin.dashboard')->domain());
    ...
    {
        $this->app->instance('request', Request::create("http://{$host}/", 'GET'));
    }
}